...
# Runtime Complexity: O (N)^2 Space Complexity: O (N) Gathers data from Package CSV File
...
# Runtime Complexity: O (N)^2 Space Complexity: O (N)^2 Gathers data from Distance Table CSV file
with open('WGUPS Distance Table.csv', 'r') as csvfile:  # https://www.programiz.com/python-programming/reading-csv-files
    reader = csv.reader(csvfile, delimiter=",")
    distance_lookup = []
    for row in reader:
        distance_lookup += [row]
# Runtime Complexity: O (N)^2 Space Complexity: O (N) Gathers data from the addresses CSV File
...
# Variables that are used in the Truck Delivery Algorithms and these lists also manually load the trucks
...
while len(truck_1) != 0:
    shortest_distance = 101.1  # Sets the Shortest distance
    for i in truck_1:  # Loops through the package locations calculating the distances and chooses the shortest distance
        ...
        if distance_1 < shortest_distance:
            ...
            low_package_id = i
    # Delivers the package at the shortest distance takes that package out of the list
...
# returns the truck to the Hub and creates a variable storing that trucks distance
...
while len(truck_2) != 0:
    shortest_distance = 101.1  # Sets the Shortest distance
    for i in truck_2:  # Loops through the package locations calculating the distances and chooses the shortest distance
...
while len(truck_3) != 0:
    shortest_distance = 101.1  # Sets the Shortest distance
    for i in truck_3:  # Loops through the package locations calculating the distances and chooses the shortest distance
        package_address = package_hash.search(i).address
...
input_start = True  # Sets Variable used in the User Interface
user_time = 1.0
user_hours = 0
user_minutes = 0


while True:  # Runs until the user asks it to stop
    if input_start:  # Instructs the user how to use the UI
    ...
    if user_input.upper() == 'TOTAL':  # Prints the total miles traveled by the truck for the User
        print("Total Miles Traveled by Trucks: {}".format(total_miles))
    elif user_input.upper() == 'STATUS':  # Lets the program know the user is looking for the status of the project
        ...
        if user_hours < 10 or (user_hours == 10 and user_minutes < 0.33):
            package_hash.search(9).address = "300 State St"
        else:
            package_hash.search(9).address = "410 S State St"
        # Runtime Complexity: O (N) Space Complexity: O (1) Package Status
        for i in range(1, 41, 1):  # Loops through all 40 packages and prints the status based on the time entered
            package_truck_location = package_hash.search(i).delivery_truck
            package_delivery_address = package_hash.search(i).address
            package_dead_line = package_hash.search(i).dead_line
            ...
            if user_time < package_delivery and user_time < package_departure:
                print("package {} still At Hub and located on {} "
                      "and will be delivered at {}:{} to {} by the dead line: {}"
                      .format(i, package_truck_location, package_delivery_hours, package_delivery_minutes,
                              package_delivery_address, package_dead_line))
            elif user_time < package_delivery:
                print("package {} is En Route and located on {} "
                      "and will be delivered at {}:{} to {} by the dead line: {}"
                      .format(i, package_truck_location, package_delivery_hours, package_delivery_minutes,
                              package_delivery_address, package_dead_line))
            elif user_time >= package_delivery:
                print("package {} was Delivered and was located on {} "
                      "and was delivered at {}:{} to {} by the dead line: {}"
                      .format(i, package_truck_location, package_delivery_hours, package_delivery_minutes,
                              package_delivery_address, package_dead_line))

    elif user_input.upper() == 'HELP':  # Instructs the user how to use the UI
        print("Welcome to the WGUPS delivery program! "
              "\n type 'total' for the total miles the trucks traveled,"
              "\n type 'status' to view the status of the packages at that time"
              "\n followed by a time in HH:MM format between the times 0:00 and 23:59"
              "\n type 'help' for these directions to be repeated,"
              "\n type 'exit' to end the program ")
    elif user_input.upper() == 'EXIT':  # Exits the Program
        break
